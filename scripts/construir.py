...
def main():
    ...
    for p in piezas:
        ...
        articulo=f'''<header class="cabecera mínima"><a class="marca" href="../index.html">Existo</a></header><main class="lectura"><a class="volver" href="../index.html">← Todos los escritos</a><article><div class="metadatos"><span>{fecha_legible(p.get('fecha',''))}</span><span>{html.escape(tipo)}</span><span>{html.escape(tema)}</span></div><h1>{html.escape(p.get('titulo','Sin título'))}</h1>{aviso}<div class="texto">{cuerpo_html(p['cuerpo'])}</div></article></main><footer>Existo · Archivo personal</footer>'''
